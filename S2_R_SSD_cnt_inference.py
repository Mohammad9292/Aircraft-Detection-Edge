...
#cap = cv2.VideoCapture('D:\Aircraft detection project\Data Collection/2021/03-05-2021 skypark sys2 arrival in sky\Gopro sys 2/right-for-presentation.mp4')

cap = cv2.VideoCapture('S2 Right/00028_Trim.mp4')
# dispW = 1920#1280#640
# dispH = 1080#720 # 480
# flip = 0
...
out = cv2.VideoWriter("S2 Right/test.mp4", fourcc, 13.0, (1920,1080))
##################################### object detection #################################################################
ODclass_names = []
...
# ODnet.setInputSize(320, 320) ##############320 320 or 160 160 or 416 416
...
def save_operation_files(text, BOX_AIRCRAFT):
    Folder = "S2 Right/"+str(datetime.datetime.now())
    os.mkdir(Folder)
    ...
    if cut_in_traffic: #sum(ii) == 0: # means cut in traffic
        ...
        if np.size(Trajectory)>2: # means Trajectory is formed
            ...
            if len(Trajectory) > 4 and len(Trajectory) <= 10:
                    ...
                    if sum(Trajectory[-5:, 0] < np.array([100, 100, 100, 100,100])) > 1 and speed > 3:  # np.std(Trajectory[:,1])<20 and     add a y threshold constraint too
                        if TRbbox[2] * TRbbox[3] < AreaLandings:
                            L += 1
                            Text2 = 'Last Operation: ' + ' LANDING'
            ...
            if len(Trajectory)>10:                                                        # ONLY FOR SYS 2##############
                ...
                if sum(  (Trajectory[-5:,0]+TRbbox[2]) >np.array([frame_width-100,frame_width-100,frame_width-100,frame_width-100,frame_width-100]))>1 and speed<-3: # np.std(Trajectory[:,1])>10 and         add y point1 > nesf ghade ax_______________
                    L+=1
