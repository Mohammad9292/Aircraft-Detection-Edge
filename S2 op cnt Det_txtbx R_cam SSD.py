...
##################################### motion detection #################################################################
#cap = cv2.VideoCapture('D:\Aircraft detection project\Data Collection/12-16-2020 skypark Sys 1 and Sys 2\CamCorder sys 2/00061_Trim.mp4') #1080 2x 160ft from runway/GH013776.mp4 #'D:/Aircraft detection project/Data Collection/10-30-2020 skypark/1080 2x 160ft from runway/GH013776.mp4'
cap = cv2.VideoCapture('D:\Aircraft detection project\Data Collection/2021/02-19-2021 spanish fork night sys1 and sys2/GH014186.mp4')
...
#out = cv2.VideoWriter("D:\Aircraft detection project\Data Collection\TESTED\layout 2\ssd/00061_Trim.mp4", fourcc, 20.0, (1920,1080))
out = cv2.VideoWriter("D:\Aircraft detection project\Data Collection/2021/02-19-2021 spanish fork night sys1 and sys2/S2_secondvid.mp4", fourcc, 10.0, (1920,1080))
##################################### object detection #################################################################
ODclass_names = []
...
configPath = 'ssd_mobilenet_v3_large_coco_2020_01_14.pbtxt'  # try different models when you do that the weights and net.setInput should be adjusted
...
ODnet.setInputSize(320, 320) ##############320 320 or 160 160
...
#ODmodel.setInputParams(size=(320, 320), scale=1/255)
###################################### tracker class ###################################################################
...
modelArchFilename = 'D:\Aircraft detection project\python murtaza\TextDetectionRecognition\Detector Models/textbox.prototxt'
modelWeightsFilename = 'D:\Aircraft detection project\python murtaza\TextDetectionRecognition\Detector Models/TextBoxes_icdar13.caffemodel'
TextRegionDetectorCNN	=	cv2.text.TextDetectorCNN_create(	modelArchFilename, modelWeightsFilename	)

OCRmodelRecognition = "D:\Aircraft detection project\python murtaza\TextDetectionRecognition\Recognition Models/CRNN_VGG_BiLSTM_CTC.onnx"
...
database = pd.read_excel ('Database.xls', sheet_name='allcountiesaddedhere')     ######################################################################### Do sth and include all counties
...
while f < length:
    ...
    if np.size(bboxs) > 2:  # means at least a box is detected (MOTION DETECTED)
        ...
        ####################################################################################################################
        start = time.time()
        ODclasses, ODconfs, ODboxes = ODnet.detect(frame, confThreshold=OD_CONFIDENCE_THRESHOLD, nmsThreshold=OD_NMS_THRESHOLD) #OD_CONFIDENCE_THRESHOLD, OD_NMS_THRESHOLD)
        ...
        if len(ODclasses) != 0:
            ...
            if len(airplane_boxes) == 1 and airplane_scores>AIRPLANE_DETECTION_CONF_BEFORE_TRACKING: # maybe 80% is a bit high, 75% could work too
                Trajectory = np.array([[0, 0]])
    ...
    if cut_in_traffic: #sum(ii) == 0: # means cut in traffic
        ...
        if np.size(Trajectory)>2: # means Trajectory is formed
            ...
            if len(Trajectory)>10:                                                        # ONLY FOR SYS 2##############
                ...
                if sum(Trajectory[-5:,0]<np.array([100,100,100,100,100]))>1 and speed>5: # np.std(Trajectory[:,1])<20 and     add a y threshold constraint too
                    D+=1
                    Text2 = 'Last Operation: ' + finalized_tail_number(TailNumberLists)[0] +' DEPARTED'
                    aircraftINFO = finalized_tail_number(TailNumberLists)[1]
                    Text3 = '# Departures : ' + str(D)
                if sum(  (Trajectory[-5:,0]+TRbbox[2]) >np.array([frame_width-100,frame_width-100,frame_width-100,frame_width-100,frame_width-100]))>1 and speed<-5: # np.std(Trajectory[:,1])>10 and         add y point1 > nesf ghade ax_______________
                    L+=1
                    Text2 = 'Last Operation: ' + finalized_tail_number(TailNumberLists)[0] +' LANDED'
                    aircraftINFO = finalized_tail_number(TailNumberLists)[1]
                    Text4 = '# Landings   : ' + str(L)
            Trajectory = np.array([[0, 0]])
            i = -1
