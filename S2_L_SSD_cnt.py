...
out = cv2.VideoWriter("S2 Left/test3.mp4", fourcc, 7.0, (1920,1080))
##################################### object detection #################################################################
ODclass_names = []
ODclassFile = 'Models/coco.names'
airplane_class_IDnum = 1
...
while f < 400:#length:
    ...
    for contour in contours:
        ...
        if cv2.contourArea(contour) < 80000 or cv2.contourArea(contour) > 500000 or w/h>5 or w/h <0.5: # SET these to an easier condition_____________________________
            continue
